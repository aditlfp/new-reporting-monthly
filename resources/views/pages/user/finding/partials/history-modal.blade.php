                ...
                <div class="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3" id="historyGrid">
                    @forelse ($findings as $finding)
                        <div class="overflow-hidden transition-all duration-300 min-h-[110px] ease-in-out bg-white border rounded-lg shadow-sm cursor-pointer border-slate-100 hover:shadow-md card-expandable"
                            data-card-id="{{ $finding->id }}">
                            <div class="p-4">
                                <div class="flex items-center justify-between">
                                    <div class="flex-1">
                                        <h4 class="font-semibold text-md text-slate-900 line-clamp-2">{{ $finding->ruangan }}</h4>
                                        <p class="text-sm text-slate-500">{{ $finding->created_at->isoformat('D MMMM Y') }}</p>
                                        <p class="text-xs text-slate-500 truncate max-w-[300px]">
                                            Di Upload Oleh : {{ $finding->user ? $finding->user->nama_lengkap : 'User Hilang' }}
                                        </p>
                                    </div>
                                ...
                                <div class="mt-4 expanded-content">
                                    <div class="grid grid-cols-1 gap-2 mb-3">
                                        <div class="overflow-hidden rounded-lg aspect-square bg-slate-100">
                                            <img data-src="{{ URL::asset('/storage/' . $finding->image_path) }}" alt="Finding Image" class="object-cover w-full h-full lazy-load">
                                        </div>
                                    </div>

                                    <div class="mb-3">
                                        <p class="mb-1 text-sm font-medium text-slate-700">Ruangan: {{ $finding->ruangan }}</p>
                                        <p class="text-sm text-slate-700 line-clamp-none">{{ $finding->note }}</p>
                                    </div>
                                </div>
                            </div>
                        </div>
